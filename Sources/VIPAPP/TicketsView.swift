    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                ...
                Spacer()
                Text(t.entry_type.displayName.uppercased())
            ...
            HStack(spacing: 12) {
                ...
                if let mins = t.rounded_minutes ?? t.elapsed_minutes ?? t.minutes {
                    Label("\(mins) min", systemImage: "clock")
                }
                if let value = t.calculated_value, !value.isEmpty {
                    Label(value, systemImage: "dollarsign.circle")
                }
                if let total = t.invoiced_total, !total.isEmpty {
                    Label("Inv \(total)", systemImage: "doc.text")
                }
                if !t.attachments.isEmpty {
                    Label("\(t.attachments.count)", systemImage: "paperclip")
                }
